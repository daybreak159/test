...
class BaseTrainer:
    ...
    def __init__(self, args, config):
        ...
        # PPO Controller
        self.controller_backend = getattr(config, 'controller_backend', 'torch')
        ...
        if self.controller_backend == 'jittor':
        ...
        if self.controller_backend == 'jittor':
            ...
            self.optimizer = jt_optim.Adam(self.controller.parameters(), lr=config.controller_lr)
        else:
        ...
        self.controller_trace_records_path = getattr(args, 'controller_trace_records', None)
        self._trace_dump_lock = threading.Lock()
        if self.controller_trace_records_path:
            trace_dir = os.path.dirname(self.controller_trace_records_path)
            if trace_dir:
                os.makedirs(trace_dir, exist_ok=True)
            with open(self.controller_trace_records_path, 'w', encoding='utf-8') as f:
                pass
        self.run_dir = getattr(args, 'run_dir', None)
        if self.run_dir is None and self.controller_trace_records_path:
            self.run_dir = os.path.dirname(self.controller_trace_records_path)
        api_cache_mode = str(getattr(args, 'api_cache_mode', 'off') or 'off').lower()
        api_cache_dir = getattr(args, 'api_cache_dir', None)
        if api_cache_mode in {'live', 'replay'}:
            os.environ['MEMSKILL_API_CACHE_MODE'] = api_cache_mode
            if api_cache_dir is None:
                api_cache_dir = os.path.join(self.run_dir or './runs', 'api_cache')
            os.environ['MEMSKILL_API_CACHE_DIR'] = api_cache_dir
            os.makedirs(api_cache_dir, exist_ok=True)
        else:
            os.environ.pop('MEMSKILL_API_CACHE_MODE', None)
    ...
    @staticmethod
    def _jsonable_exec_result(result):
        return {
    ...
    def _dump_controller_trace_records(self, steps: List[Dict], local_buffer: PPOBuffer,
                                       episode_summary: Dict[str, Any]) -> None:
        if not self.controller_trace_records_path:
            return

        n = min(len(steps), len(local_buffer.states))
        if n == 0:
            return

        try:
            returns, advantages = local_buffer.compute_returns_and_advantages(
        ...
        for i in range(n):
            step = steps[i]
            selected_skills = step.get('selected_op')
            if selected_skills is None:
                selected_skills = step.get('selected_skills', [])
            if isinstance(selected_skills, str):
                selected_skills = [selected_skills]
            exec_results = step.get('exec_results_structured', [])
            executor_actions = [
                r.get('action_type')
                for r in exec_results
                if isinstance(r, dict) and r.get('action_type') is not None
            ]
            qa_reward = float(episode_summary.get('final_reward', 0.0))
            total_reward = float(local_buffer.rewards[i])
            record = {
                'source': 'memskill-api-cache',
                'episode_id': episode_summary.get('episode_id'),
                'trace_id': episode_summary.get('episode_id'),
                'session_id': step.get('session_idx'),
                'span_id': i,
                'step': i,
                'session_idx': step.get('session_idx'),
                'session_text': step.get('session_text', ''),
                'state_embedding': self._jsonable_array(local_buffer.states[i]),
                'op_embeddings': self._jsonable_array(local_buffer.op_embs[i]),
                'candidate_ops': step.get('candidate_ops', []),
                'selected_action': self._jsonable_array(local_buffer.actions[i]),
                'action_idx': self._jsonable_array(local_buffer.actions[i]),
                'selected_op': step.get('selected_op'),
                'selected_skills': selected_skills,
                'log_prob': float(local_buffer.log_probs[i]),
                'old_log_prob': float(local_buffer.log_probs[i]),
                'new_log_prob': step.get('new_log_prob'),
                'value': float(local_buffer.values[i]),
                'advantage': float(advantages[i]) if advantages is not None else None,
                'return': float(returns[i]) if returns is not None else None,
                'new_op_mask': self._jsonable_array(local_buffer.new_op_masks[i]),
                'process_reward': float(step.get('process_reward', 0.0)),
                'qa_reward': qa_reward,
                'total_reward': total_reward,
                'reward': total_reward,
                'done': bool(local_buffer.dones[i]),
                'retrieved_memories': step.get('retrieved_memories', []),
                'retrieved_indices': self._jsonable_array(step.get('retrieved_indices', [])),
                'exec_results': exec_results,
                'executor_actions': executor_actions,
                'parse_total': int(step.get('parse_total', 0)),
                'parse_fail_count': int(step.get('parse_fail_count', 0)),
                'parser_success': bool(step.get('parser_success', False)),
                'api_call_success': bool(step.get('api_call_success', False)),
                'apply_success': bool(step.get('apply_success', False)),
                'memory_bank_size_before': step.get('memory_bank_size_before'),
                'memory_bank_size_after': step.get('memory_bank_size_after'),
            }
            if i == n - 1:
                record['episode'] = episode_summary
            records.append(record)

        with self._trace_dump_lock:
            with open(self.controller_trace_records_path, 'a', encoding='utf-8') as f:
    ...
    def train_episode(self, conversation_data: Dict, memory_bank: MemoryBank,
        ...
        if precompute_embeddings and isinstance(sessions, list):
            ...
            # session_embeddings 是当前 trace 中每个 span/session 的文本向量。
            # 它后续会作为 MemoryBank 检索 query，并参与构造 PPOBuffer.states 中的 state_embedding。
        ...
        # 一整条 trace 的所有 session 都处理完后，才用最终 MemoryBank 回答 QA。
        # qa_performance 是 episode 级 final reward，稍后会通过 finish_episode 回填到 PPOBuffer.rewards。
        ...
        # 将 final QA reward 合并进当前 episode 的 reward 序列，并把最后一个 done 标记为 True。
        # 如果开启 redistribute_reward，final reward 会按指数衰减分配给该 trace 的多个 step。
        ...
        episode_summary = {
            ...
            'qa_prompts': [],
            'qa_responses': [],
            'f1_score': qa_performance,
            'llm_judge_score': 0.0,
        }
        self._dump_controller_trace_records(episode_log['steps'], ppo_buffer, episode_summary)
    ...
    def _process_session(self, session_text: str, memory_bank: MemoryBank,
        ...
        """
        step_log = {}
        step_log['memory_bank_size_before'] = len(getattr(memory_bank, 'memories', []) or [])

        # Use pre-computed state encoder embedding if provided, otherwise compute on-the-fly
        # session_embedding 是当前 span/session 的文本语义向量。
        # 它不是最终 state，最终 state 还需要加入当前 MemoryBank 中检索到的相关 memories。
        ...
        # 使用当前 session_embedding 做向量检索，得到与当前 span 最相关的历史记忆。
        # retrieved_memory_embeddings 会被 StateEncoder 融合进 state_embedding。
        ...
        # Encode state
        # state_embedding 对应 PPO 的状态 s_t：
        #   state_embedding = [session_embedding || fused_memory_embedding]
        # 它会写入 PPOBuffer.states，并作为 controller.state_net 的输入。
        ...
        # candidate_ops 是当前 step 的动态动作空间 A_t。
        # 它来自 OperationBank，包含初始 memory skills 和 Designer 后续新增/更新的 skills。
        ...
        # op_embeddings 的每一行对应一个 candidate_ops 中的 MemSkill。
        # 这些向量会写入 PPOBuffer.op_embs，后续和 state_embedding 配对打分。
        ...
        if len(new_op_indices) > 0:
            # new_op_mask 与 op_embeddings 的行一一对应。
            # 1 表示该 skill 是 Designer 新增/刚更新的候选动作，用于新 skill 探索 bias。
        ...
        # controller 输入 state_embedding/op_embeddings/new_op_mask，输出:
        #   action_idx: 选中的 skill 下标或 Top-K 下标列表
        #   log_prob: rollout 时旧策略选择该 action/Top-K action 的 log 概率
        #   value: critic 对当前状态 V(s_t) 的预测
        if self.controller_backend == 'jittor':
            action_idx, log_prob, value = self.controller.select_action(
                state_embedding, op_embeddings, deterministic=False, new_op_mask=new_op_mask
            )
        else:
            # Use no_grad during rollout to avoid unnecessary graph construction
            # PyTorch baseline 中先把 numpy 向量转成 torch.Tensor。
            # rollout 阶段只记录轨迹，不做反向传播，所以使用 no_grad。
        ...
        # action_idx 是向量动作空间中的下标；这里把下标映射回具体 Operation 对象交给 Executor。
        ...
        step_log['exec_results_structured'] = [
            self._jsonable_exec_result(r) for r in exec_results
        ]
        step_log['parse_total'] = len(exec_results)
        step_log['parse_fail_count'] = sum(1 for r in exec_results if not r.success)
        step_log['executor_actions'] = [
            getattr(r, 'action_type', None) for r in exec_results
            if getattr(r, 'action_type', None) is not None
        ]
        step_log['parser_success'] = (
            step_log['parse_total'] > 0 and step_log['parse_fail_count'] == 0
        )
        step_log['api_call_success'] = not any(
            'API call failed' in str(getattr(r, 'reasoning', ''))
            for r in exec_results
        )

        # process_reward 是当前 step 的即时奖励。
        # 它比较 controller 选择的 skill 类型与 Executor 实际成功执行的操作类型是否匹配。
        ...
        #   value       -> values
        #   reward      -> rewards
        #   new_op_mask -> new_op_masks
        # push 内部会先把 done 写为 False，episode 结束后由 finish_episode 改最后一步。
        ppo_buffer.push(
        ...
        step_log['memory_bank_size_after'] = len(getattr(memory_bank, 'memories', []) or [])
    ...
    def _score_qa_responses(self, qa_list: List[Dict], valid_qa_indices: List[int],
        ...
        # 保存每条 QA 的 F1 分数列表，用于最后求当前 conversation 的平均 reward。
        f1_scores = []
        # predictions 用原始 qa_idx 做 key，便于后面组装 DesignerCase 时按问题索引取回答。
        predictions = {}
        # f1_scores_map 同样用原始 qa_idx 做 key，避免采样后顺序变化导致分数对不上问题。
        f1_scores_map = {}

        # ret 是 get_llm_response() 的返回结果。
        # 每个元素的 i 对应 qa_list 中的原始问题下标，response 是回答模型输出。
        for i, response, _, success in ret:
            # 用原始下标取回 QA 数据，这里包含 question、answer/answers、evidence、category 等字段。
            qa = qa_list[i]
            # evaluator 负责兼容不同数据集的标准答案字段，例如 answer 或 answers。
            ground_truth = self.evaluator.get_ground_truth(qa)
            # 如果 LLM 调用失败，prediction 记为空字符串，后续 F1 会自然变低。
            prediction = response.strip() if success and response is not None else ""
            # HotpotQA 的回答可能带解释或格式噪声，这里用 evaluator 的抽取器提取最终答案。
            ...
            # 计算当前问题的自动指标分数，这是 Designer 判断失败样本的重要依据之一。
            f1 = self.evaluator.compute_f1(prediction, ground_truth, qa)
            # f1_scores 保留顺序列表，供 avg_reward 计算。
            f1_scores.append(f1)
            # predictions/f1_scores_map 保留 qa_idx -> 结果 的映射，供 DesignerCase 打包使用。
            predictions[i] = prediction
            f1_scores_map[i] = f1

        # 如果 reward_metric 使用 llm_judge，则还要再调一次 judge 模型给回答打分。
        llm_judge_scores = {}
        if reward_metric == 'llm_judge':
            judge_task_args = []
            # valid_qa_indices 是本轮实际参与评估的原始 QA 下标。
            for qa_idx in valid_qa_indices:
                qa = qa_list[qa_idx]
                ground_truth = self.evaluator.get_ground_truth(qa)
                prediction = predictions.get(qa_idx, "")
                # evaluator 构造 judge prompt，把 question、标准答案、模型回答一起交给 judge。
        ...
        # 当前训练 reward 可以用 LLM judge 均值，也可以默认用 F1 均值。
        if reward_metric == 'llm_judge':
        ...
        # 返回给 _evaluate_qa：
        # - avg_reward 用于训练/日志
        # - predictions/f1_scores_map/llm_judge_scores 用于打包 DesignerCase
        # - f1_scores 是原始分数列表，当前调用点没有继续使用
    ...
    def _evaluate_qa(self, conversation_data: Dict, memory_bank: MemoryBank,
        ...
        # evaluator 根据当前数据集和参数生成评估用 args，例如 retriever、模型接口参数等。
        eval_args = self.evaluator.prepare_eval_args()

        # 流程第 1 步：原始 conversation_data -> qa_list。
        # data_processor 只负责从不同数据集格式里取出 QA 列表，具体字段通常包含 question/answer/evidence/category。
        qa_list = self.data_processor.get_qa_list(conversation_data)
        if len(qa_list) == 0:
            # 有些非 QA 任务可能没有 qa 字段，但 evaluator 能直接给 episode reward。
            if hasattr(self.evaluator, "get_episode_reward"):
                reward = float(self.evaluator.get_episode_reward(conversation_data))
                return reward, reward
            return 0.0, 0.0

        # evaluator 先过滤不可用于训练评估的问题，例如格式不完整或数据集特定的跳过项。
        # 返回值通常是 [(qa_idx, qa_item), ...]，保留 qa_idx 是为了后面映射回 qa_list。
        valid_qa = self.evaluator.filter_qa_list(qa_list)
        if len(valid_qa) == 0:
            return 0.0, 0.0

        # 对训练阶段的 QA 做采样，避免每个 conversation 的所有问题都参与训练评估。
        # sampled_valid_qa 仍然保留原始 qa_idx，后续 DesignerCase 的 query_id 会用到它。
        ...
        # LoCoMo 支持按比例采样训练 QA，这段只是记录采样前后的类别分布，方便检查采样是否偏斜。
            ...
            if sampling_ratio < 1.0:
                def _category_counts(items):
                    ...
                    for _, qa_item in items:
                        # LoCoMo 的 category 表示问题类型，缺失或非法时默认归到 1。
                        try:
        ...
        # 后续所有评估都只在 sampled_valid_qa 上进行。
        valid_qa = sampled_valid_qa

        # valid_qa_indices 保存原始 QA 下标，例如 conv-26_2 里的 2 就来自这里。
        valid_qa_indices = [idx for idx, _ in valid_qa]
        # 只取问题文本做 query embedding，用于从当前 memory_bank 检索相关 memories。
        questions = [qa['question'] for _, qa in valid_qa]

        # 流程第 2 步的准备：为每个 question 批量计算检索向量。
        # 这里用 eval_args.retriever 对问题编码，和 memory_bank 里的 retriever embedding 对齐。
        ...
        # task_args 会交给 get_llm_response() 并行调用回答 LLM。
        task_args = []
        # retrieval_info 会在组装 DesignerCase 时使用，记录每个问题实际检索到了哪些 memory。
        retrieval_info = {}  # qa_idx -> (retrieved_mems, retrieved_indices)

        # 流程第 2 步：每个 question 检索 memory_bank。
        for idx, (qa_idx, qa) in enumerate(valid_qa):
            question = qa['question']
            # q_embeddings 的顺序和 valid_qa 的顺序一致，因此用 idx 取当前问题向量。
            q_embedding = q_embeddings[idx]

            # 从当前 episode 已构建出的 memory_bank 中取 top-k 相关 memory。
            # use_state_encoder=False 表示 QA 评估使用 retriever embedding，而不是 controller 的 state encoder。
            retrieved_mems, retrieved_indices = memory_bank.retrieve(
                q_embedding, use_state_encoder=False
            )
            # 保存检索结果，后面 DesignerCase 会把 retrieved_memories/retrieved_indices 一起存下来。
            retrieval_info[qa_idx] = (retrieved_mems, list(retrieved_indices))

            # evaluator 把 question、retrieved memories 和原始 qa 字段拼成回答模型的 prompt。
            prompt = self.evaluator.build_prompt(question, retrieved_mems, qa)
            # task_args 中保留 qa_idx，LLM 返回后可以知道这个 response 属于哪道题。
            task_args.append((qa_idx, prompt, eval_args))

        # 流程第 3 步：LLM 生成 prediction。
        if len(task_args) == 0:
            return 0.0, 0.0

        # 批量调用回答 LLM。ret 中每条记录会包含 qa_idx、response、附加信息和 success 标记。
        ret = get_llm_response(args=eval_args, task_args=task_args)

        # 流程第 4 步的配置：确定 reward 用 F1 还是 LLM judge。
        reward_metric = getattr(self.config, 'reward_metric', 'f1').lower()
        # 只有启用 Designer 时才需要收集 DesignerCase。
        collect_cases = (self.designer is not None)
        # F1 模式下，低于 designer_f1_threshold 的样本会被视为失败 case。
        f1_threshold = getattr(self.config, 'designer_f1_threshold', 0.5)
        need_llm_judge = reward_metric == 'llm_judge'

        # 如果要收集 DesignerCase，就把当前 memory_bank 完整序列化一次。
        # 这个快照表示回答这些 QA 时系统拥有的 memory 状态，便于后续分析或复现。
        memory_snapshot = memory_bank.to_dict() if collect_cases else None

        # 流程第 4 步：evaluator 计算 f1 / judge score。
        # 同时得到每个 qa_idx 对应的 prediction 和分数，用来打包 DesignerCase。
        ...
        if collect_cases:
            # 流程第 5 步：把原始 QA、检索结果、模型回答、评分结果合并成 DesignerCase。
            for qa_idx in valid_qa_indices:
                # qa 是原始 QA 条目，提供 question/evidence/category/answer 等数据集字段。
                qa = qa_list[qa_idx]
                # 取回刚才为这个 question 检索到的 memories 和它们在 memory_bank 中的下标。
                ...
                # evaluator 统一处理不同数据集的答案字段，例如 answer、answers 或其它变体。
                case_ground_truth = self.evaluator.get_ground_truth(qa)
                # DesignerCase 中 ground_truth 存字符串，便于 JSON 序列化和 prompt 展示。
                ...
                # 根据当前 reward metric 判断这条 QA 是否答对。
                # llm_judge 模式要求 judge_score == 1.0；F1 模式要求 f1 达到阈值。
                if need_llm_judge:
                    is_correct = (judge_score == 1.0)
                else:
                    is_correct = (f1 >= f1_threshold)

                # DesignerCase 是 Designer 的核心输入数据结构：
                # 它把“问了什么、标准答案、模型答了什么、检索到了什么、当时 memory_bank 长什么样”
                # 放到同一条记录里，方便 Designer 后续判断失败原因。
                case = DesignerCase(
                    # query_id 用 conversation_id + 原始 qa_idx 保证同一问题跨轮可去重。
                ...
                # 流程第 6 步：CaseCollector.add_case() 只会真正保留失败 case。
                self.designer.case_collector.add_case(case)

        # avg_reward 同时作为训练 reward 和原始 performance 返回。
    ...
    def update_controller(self, ppo_buffer: PPOBuffer) -> Dict:
        ...
        # 这里读取 PPOBuffer.rewards / values / dones，从后往前计算:
        #   returns: 当前 step 后续累计收益
        #   advantages: 实际收益相对 critic 预测 V(s_t) 的超额部分
        # 这一步仍然是 NumPy 逻辑，无需梯度；真正的梯度更新在 compute_ppo_loss 中发生。
        ...
        # Get batch data
        # batch 是 PPOBuffer 中保存的一批 rollout 轨迹:
        #   states / op_embs / new_op_masks 是重新计算新策略输出所需的输入向量
        #   actions / log_probs / values 是 rollout 当时旧策略留下的训练基准
        ...
        for epoch in range(self.ppo_epochs):
            ...
            for start in range(0, n_samples, minibatch_size):
                ...
                # Extract minibatch
                # 每个样本的候选 skill 数量可能不同，所以 op_embs/new_op_masks 保留 list 形式。
                # 真正进入网络前，会在 controller.compute_ppo_loss 中 padding 成统一 batch tensor。
                mb_batch = {
                ...
                # PyTorch 和 Jittor controller 使用同一个接口:
                #   输入旧轨迹字段 + returns/advantages
                #   重新计算 new_log_probs / new values / entropy
                #   得到 PPO clipped loss、value loss 和 entropy bonus
    ...
    def train(self, train_data: List[Dict]):
        ...
        if self.run_dir:
            try:
                from jittor_controller_repro.online_artifacts import generate_online_run_artifacts
                artifacts = generate_online_run_artifacts(
                    run_dir=self.run_dir,
                    training_logs=self.training_logs,
                    args=self.args,
                    config=self.config,
                    backend=self.controller_backend,
                    step_records_path=self.controller_trace_records_path,
                )
                self.log(f"Generated online run artifacts: {artifacts}")
            except Exception as exc:
                self.log(f"Failed to generate online run artifacts: {exc}", level='warning')
    ...
    def _tree_to_numpy(value):
        """Convert backend tensor leaves to NumPy/Python leaves for checkpoints."""
        if torch.is_tensor(value):
            return value.detach().cpu()
        if hasattr(value, "numpy") and value.__class__.__module__.startswith("jittor"):
            return value.numpy()
        if isinstance(value, dict):
            return {k: BaseTrainer._tree_to_numpy(v) for k, v in value.items()}
        if isinstance(value, list):
            return [BaseTrainer._tree_to_numpy(v) for v in value]
        if isinstance(value, tuple):
            return tuple(BaseTrainer._tree_to_numpy(v) for v in value)
        return value

    def _controller_state_for_checkpoint(self):
        state = self.controller.state_dict()
        if self.controller_backend == 'jittor':
            return self._tree_to_numpy(state)
        return state

    def _optimizer_state_for_checkpoint(self):
        if self.controller_backend == 'jittor':
            # Jittor optimizer state contains references to live parameter Vars.
            # Store controller weights as the reproducible state; recreate Adam on resume.
            return None
        return self.optimizer.state_dict()

    def _load_controller_state_from_checkpoint(self, checkpoint):
        state = checkpoint['controller_state_dict']
        if self.controller_backend == 'jittor':
            import jittor as jt
            state = {
    ...
    def save_checkpoint(self, epoch: int):
        ...
        checkpoint = {
            ...
            'controller_state_dict': self._controller_state_for_checkpoint(),
            'optimizer_state_dict': self._optimizer_state_for_checkpoint(),
    ...
    def load_checkpoint(self, path: str):
        ...
        self._load_controller_state_from_checkpoint(checkpoint)
        if self.controller_backend == 'jittor':
            saved_backend = checkpoint.get('controller_backend', 'torch')
            if saved_backend != 'jittor':
                self.log(
                    f"Checkpoint controller_backend={saved_backend}; cannot load into Jittor backend.",
                    level='warning'
                )
            elif checkpoint.get('optimizer_state_dict') is None:
                self.log("Jittor optimizer state not stored; optimizer reinitialized.", level='warning')
            else:
