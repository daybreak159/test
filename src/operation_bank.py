...
class OperationBank:
    ...
    def _recompute_embeddings(self):
        ...
        # 每个 Operation/MemSkill 的文本描述会被编码成一个 op_embedding。
        # 这些向量在 trainer._process_session 中被堆叠成 op_embeddings，
        # 对应 PPOBuffer.op_embs，也就是 controller 的动态动作空间 A_t。
        embeddings = self.encoder.encode(texts)

        # Assign embeddings to operations
        # OperationBank 只保存每个 skill 自己的 embedding。
        # 到具体 step 时，candidate_ops 的顺序决定 action_idx 与 skill 名称之间的映射。
    ...
    def get_candidate_operations(self) -> List[Operation]:
        ...
        # 返回顺序必须稳定，因为 controller 输出的是 action_idx。
        # 例如 action_idx=2 的含义取决于当前 candidate_ops[2] 是哪个 MemSkill。
